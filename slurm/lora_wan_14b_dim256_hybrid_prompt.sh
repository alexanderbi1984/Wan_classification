#!/bin/bash -l
#SBATCH --job-name=WanLoRA14B_dim256_hybrid_prompt
...
echo "[$(date +%T)] Experiment: dim256 + hybrid CE(0.8)+CORAL(0.2) + prompt conditioning"
echo "[$(date +%T)] Config: config_pain/config_lora_dim256_hybrid_prompt.yaml"

srun --ntasks=1 --gpus-per-node=${SLURM_GPUS_PER_NODE} \
  torchrun --standalone --nproc_per_node=${SLURM_GPUS_PER_NODE} \
  train_online_wan.py \
    --config config_pain/config_lora_dim256_hybrid_prompt.yaml \

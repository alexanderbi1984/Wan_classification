#!/bin/bash -l
#SBATCH --job-name=WanLoRA14B_bin_bl1pa4
...
echo "[$(date +%T)] Experiment: Binary BL1 vs PA4 (CE loss)"
echo "[$(date +%T)] Config: config_pain/config_lora_binary_bl1_pa4.yaml"

srun --ntasks=1 --gpus-per-node=${SLURM_GPUS_PER_NODE} \
  torchrun --standalone --nproc_per_node=${SLURM_GPUS_PER_NODE} \
  train_online_wan.py \
    --config config_pain/config_lora_binary_bl1_pa4.yaml \

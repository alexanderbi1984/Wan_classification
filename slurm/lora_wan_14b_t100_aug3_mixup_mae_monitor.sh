#!/bin/bash -l
#SBATCH --job-name=WanLoRA14B_t100_aug3_mixup_mae
...
echo "[$(date +%T)] ABLATION: t100 + aug3 + MixUp, monitor=val_pain_MAE (vs QWK)"
echo "[$(date +%T)] Config: config_pain/config_lora_t100_aug3_mixup_mae_monitor.yaml"

srun --ntasks=1 --gpus-per-node=${SLURM_GPUS_PER_NODE} \
  torchrun --standalone --nproc_per_node=${SLURM_GPUS_PER_NODE} \
  train_online_wan.py \
    --config config_pain/config_lora_t100_aug3_mixup_mae_monitor.yaml \

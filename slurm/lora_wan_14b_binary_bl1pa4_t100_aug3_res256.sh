#!/bin/bash -l
#SBATCH --job-name=WanLoRA14B_bin_bl1pa4_t100aug3_res256
...
echo "[$(date +%T)] Experiment: Binary BL1 vs PA4 + t100 + aug3 + 256x256"
echo "[$(date +%T)] Config: config_pain/config_lora_binary_bl1_pa4_t100_aug3_res256.yaml"
echo "[$(date +%T)] seq_len: 4096 (vs 1024 at 128x128), batch=4, accum=6"

srun --ntasks=1 --gpus-per-node=${SLURM_GPUS_PER_NODE} \
  torchrun --standalone --nproc_per_node=${SLURM_GPUS_PER_NODE} \
  train_online_wan.py \
    --config config_pain/config_lora_binary_bl1_pa4_t100_aug3_res256.yaml \

#!/bin/bash -l
#SBATCH --job-name=WanLoRA14B_t100_prompt
...
echo "[$(date +%T)] Experiment: t100 + aug2 + T5 prompt conditioning"
echo "[$(date +%T)] Config: config_pain/config_lora_t100_prompt.yaml"

srun --ntasks=1 --gpus-per-node=${SLURM_GPUS_PER_NODE} \
  torchrun --standalone --nproc_per_node=${SLURM_GPUS_PER_NODE} \
  train_online_wan.py \
    --config config_pain/config_lora_t100_prompt.yaml \

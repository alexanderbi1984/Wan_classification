#!/bin/bash -l
#SBATCH --job-name=WanLoRA14BWAugMix
...
echo "[$(date +%T)] Wan 14B DiT LoRA fine-tuning for BioVid pain classification (4-GPU DDP)"
echo "[$(date +%T)] Config: config_pain/config_lora.yaml"
echo "[$(date +%T)] LoRA rank=4 on q,k,v,o — ~6.6M trainable params (0.05% of 14B)"
echo "[$(date +%T)] Gradient checkpointing: ON | Precision: bf16-mixed"
